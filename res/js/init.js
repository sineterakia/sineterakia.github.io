...
var jAlbumGlobals = (function () {
	...
	var credits = "${creditText}"; // should skin show credits in footer
	var albumTitle = "${albumTitle}"; // what's the album title?
	var contentPath = "${contentPath}"; // path points to the skin folder
	var imgHoverScaleFactor = "${imgHoverScaleFactor}"; // scale factor of hovering an element

	var folderTitleUp = ${folderTitleUp}; // positioning of folder title, under or above folder
	var folderImgCount = ${folderImgCount}; // says if folder deep count should be shown
	var maxImgInRow = ${maxImgInRow}; // restricts the number of elements in a row
	var imgBorderSize = ${imgBorderSize}; // define border size of element
	var imgBoxSize = ${imgBoxSize}; // defines size of container of element
	var showFolderName = ${showFolderName}; // says if folder name should be displayed
	var textSize = ${textSize}; // defines text size of whole page
	var showComments = ${showComments}; // defines if comments should be shown in slideshow
	var slideBorderSize = ${slideBorderSize};
	var slideBorderColor = "${slideBorderColor}";
	var style = "${style}";
...
var jAlbumInject = (function () {
	...
	inj('</div><div class="center"><p><a href="http://jalbum.net/">${creditText}</a> - '); // credits in footer
...
if(!document.getElementById('non-embedded')){ // check if embedded
	...
	${widgetJavaScript} // get JavaScript code for widget
}
